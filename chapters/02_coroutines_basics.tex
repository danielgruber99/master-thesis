...
Pointer chasing data structures, like chaining or linear probing hash tables, and tree-based structures, e.g. B+-trees,
typically suffer from cache misses as their access patterns are irregular and non-contiguous.
...
Thus, when traversing these data structures, the CPU frequently encounters cache misses, leading to significant performance degradation as it waits for data to be fetched from main memory.
...
The hardware prefetcher in modern CPUs is often ineffective for these pointer-chasing patterns due to their irregular access nature, providing no predictable stride or pattern for the prefetcher to exploit.
...
However, by overlapping multiple independent operations, for example with AMAC, GP or Coroutines, software prefetching can be effectively utilized to hide the latency of these cache misses.
...
% Group prefetching and software pipelined prefetching [5] overlap multiple hash table lookups to accelerate hash joins.
% After a prefetch is issued, the control flow switches to execute the computation stage of another operation.
% Asynchronous memory access chaining (AMAC) [26] is a general approach that allows one to transform a heterogeneous
% set of operations into state machines to facilitate switching between operations upon cache misses.
% A notable drawback of theseapproaches is they require developers hand-craft algorithms. The resulted code is typically not intuitive to understand and hard to maintain.
% This limits their application to simple or individual data structures and operations (e.g., tree traversal). or coroutines.

% Consider traversing a tree-like data structure: the immediate succession of identifying and accessing a node leaves negligible time for the hardware to load the data, even with prefetch hints.
...
Before diving into the details of software prefetching, a brief illustration of how data is loaded through the memory subsystem is provided.
...
% MoreoverIn particular, in Intel processors the transfer of data from L2 to L1 Cache goes through the Line Fill Buffer (LFB) , a hardware component to manage cache line
% transfers between L1 and L2 cache.
...
Note, that on amd platforms the LFB is referred to as Miss Address Buffer (MAB) whereas LFB is the terminology used in Intel architectures.
