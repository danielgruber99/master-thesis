...
\subsection{Cause of Cache Misses Data Structures}\label{subsec:CauseOfCacheMissesInDataStructures}
...
To traverse such a tree from its root node to the target leaf node, the accessing thread must dereference multiple pointers sequentially from the root to the leaf, forming a random access pattern.

This pointer chasing behavior poses a significant challenge for hardware prefetchers.
Such access patterns are very difficult, if not impossible, for hardware prefetchers to predict accurately, leading to a high likelihood of last-level cache misses upon each pointer dereference.
As a result, in main-memory systems, data stalls often dominate total CPU execution time.
Beyond tree-based indexes, version chains in multi-versioned concurrency control systems can also cause a significant amount of data stalls when the accessing thread searches for a particular version.
In some systems, over 50\% of the total CPU cycles can be spent waiting for data to be loaded from memory to CPU caches.
Hiding such stalls can potentially lead to much higher overall performance.


% TODO: make my example also with hashtable and pictures


\subsection{Techniques to counter Cache Misses}\label{subsec:TechniquesCounterCacheMisses}


%Basic Idea. Software prefetching techniques [5, 21, 26, 44] use
% workload semantics to issue prefetch instructions [18] to explicitly
% bring data into CPU caches. Worker threads handle requests (e.g.,
% tree search) in batches. To access data (e.g., a tree node) in request
% 𝑡1 which may incur a cache miss, the thread issues a prefetch and
% switches to another request 𝑡2, and repeats this process. While the
% data needed by 𝑡1 is being fetched from memory to CPU cache,
% the worker thread handles 𝑡2, which may further cause the thread
% to issue prefetch and switch to another request. By the time the
% worker thread switches back to 𝑡1, the hope is that the needed
% data is (already and still) cache-resident. The thread then picks up
% at where it left for 𝑡1, dereferences the pointer to the prefetched
% data and continues executing 𝑡1 until the next possible cache miss
% upon which a prefetch will be issued. It is important that the
% switching mechanism and representation of requests are cheap and
% lightweight enough to achieve a net gain
...
The first approach aims to eliminate cache misses by increasing spatial and temporal locality.
Locality can be improved through several methods: eliminating indirection by designing cache-conscious data structures, matching the data layout to the access pattern of the algorithm such that data accessed together is stored in contiguous space, or reorganizing memory accesses to increase locality through techniques like array blocking and tree blocking.
However, these optimizations often require fundamental changes to data structure design and may not be applicable when the data structure layout is already optimized or when modifications would negatively impact other operations.
...
This approach increases instruction-level parallelism and leads to more effective out-of-order execution.
To reduce the main-memory access penalty, a non-blocking load must be introduced early enough to allow independent instructions to execute while fetching data.
This can be achieved through simple prefetching within an instruction stream or by exploiting simultaneous multithreading with helper threads that prefetch data.
However, in scenarios like index lookups where one memory access depends on the previous one with few independent instructions in between, these techniques provide limited benefits.

The third approach seeks to hide the cache miss penalty by overlapping memory accesses.
Modern memory systems can serve multiple memory requests in parallel (typically around 10 concurrent requests in current Intel CPUs),
and exploiting this memory-level parallelism increases memory throughput.
However, overlapping requires independent memory accesses that can be issued simultaneously.
In the access chain of an index lookup, where each step depends on the result of the previous one,
such independent accesses are not naturally available.

These traditional approaches provide limited benefits for individual index lookups with dependent memory accesses.
...
% TODO: AMAC AND GROUP PREFETCHING
Hand-Crafted Approaches. The mechanism we just described
fits naturally with many loop-based operations. Group prefetching
and software pipelined prefetching [5] overlap multiple hash table
...
structures and operations (e.g., tree traversal). Recent approaches
tackle this challenge using lightweight coroutines, described next.
...
Typically, these hints are communicated via specific
instructions embedded within modern ISAs such as x86 and ARMv8.
Especially in the context of trees and hash tables, many studies
...
% However, implementing effective software prefetching presents several challenges.
...
Modern platforms offer several prefetch instructions to target different levels of the cache hierarchy, with each instruction initiating the transfer of one cache line.
...
This procedure happens synchronously—if the translation is not cached within the Translation Lookaside Buffer (TLB), the CPU contacts a Page Miss Handler (PMH) before continuing execution of the prefetch instruction.

% ERROR: Figure conflicts with previous figure, i think first TLB is asked, then caches, and if miss, PMH is asked
% \begin{figure}[h]
%   \centering
%   \includegraphics[width=0.6\paperwidth]{figures/sequenzdiagramm_data_load.png}
