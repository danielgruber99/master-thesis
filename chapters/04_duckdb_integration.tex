...
DuckDB uses for the hash join a linear probing hashtable implementation.
The hashtable is implemented in the files \texttt{join\_hashtable.cpp} and \texttt{join\_hashtable.hpp}.
The hashtable is divided into two phases: the \textit{build phase} and the \textit{probe phase}.
In the build phase, the hashtable is constructed by inserting all tuples from the build side of the join into the hashtable.
In the probe phase, the hashtable is queried with tuples from the probe side of the join to find matching entries.
The hashtable uses linear probing to resolve collisions, meaning that if a hash index is already occupied, the next available index is checked until an empty slot is found.
...
struct Silent {
	...
	struct promise_type {
		...
		Silent get_return_object() noexcept { return Silent {handle_type::from_promise(*this)};}
		void return_void() noexcept { }
	};
	handle_type h_;
	Silent(handle_type h) : h_(h) { }
	~Silent() { h_.destroy(); }
};

template <bool USE_SALTS, bool HAS_SEL, const size_t numCoroutines>
Silent ProbeForPointersInternalCoro(JoinHashTable::ProbeState &state, JoinHashTable &ht, ht_entry_t *entries,
                                    Vector &pointers_result_v, const SelectionVector *row_sel, idx_t &count,
                                    idx_t &keys_to_compare_count, const size_t offset) {
