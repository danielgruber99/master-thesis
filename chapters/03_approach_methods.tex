...
lookupEntries(datastructure ds, lookupsVector lookups) noexcept {
    ...
    std::array<Task, numCoroutines> coroutines =
        ([&ht, &lookups, &result]<size_t... Is>(std::index_sequence<
            Is...>) {return std::array{lookup<numCoroutines>(ht, lookups,
             result, Is)...};})(std::make_index_sequence<numCoroutines>{});
    while (true) {
...
\subsection{The Coroutine Wrapper class}\subsection{subsec:CoroutineWrapper}
The implemented Coroutine Wrapper, shown in \autoref{lst:scheduler}, is a simple struct that encapsulates the coroutine handle and manages its lifecycle.
As explained in \autoref{sec:CppCoroutines}, the coroutine wrapper has a nested promise\_type struct that defines the behavior of the coroutine,
including the initial and final suspension points, exception handling, and return object creation.
In this case, the coroutine is designed to never suspend initially (initial\_suspend returns std::suspend\_never), and thus is eagerly started upon creation.
The final suspension point (final\_suspend) returns std::suspend\_always, meaning the coroutine will suspend itself when it reaches the end of its execution,
allowing the scheduler to manage its completion and destruction appropriately.
Furthermore, the wrapper class has a handle\_type member that holds the coroutine handle,
which the scheduler uses to resume the coroutine and check its completion status.

\begin{lstlisting}[language=C++, frame=single, caption={Scheduler for lookup}, label={lst:scheduler}, numbers=left]
struct Task {
    struct promise_type;
    ...
    handle_type h_; // Handle of the coroutine
...
For scheduling multiple coroutines, the scheduler from \autoref{lst:scheduler} is used.
...
template<const size_t numCoroutines>
Task lookup(const ht_primary_key::Hashtable &ht, const auto &lookups,
...
\begin{lstlisting}[language=C, frame=single, caption={linear probing hashtable coroutine lookup}, label={lst:linear_probing_ht_lookup_coro}, numbers=left]
template<const size_t numCoroutines>
Task lookup_ht_entry_t(const LinearProbingHashTable<uint64_t, uint64_t>
        &ht, const auto &lookups, auto &results, const size_t offset) {
    auto idx = offset; size_t key; uint64_t hashIdx; uint64_t hash;
    LinearProbingHashTable<uint64_t, uint64_t>::EntryData *data = nullptr;

    for (; idx < lookups.size(); idx += numCoroutines) {
        key = lookups[idx]; hash = ht.ComputeHash(key);
        uint64_t slot = hash & ht.bitmask;
        uint64_t salt = ht.ExtractSalt(hash);

        ht_entry_t entry;
        while (true) {
            PREFETCH(&(ht.entries[slot]));
            co_await std::suspend_always{};
            ...
            if (ht.use_salts) {
                if (entry.GetSalt() == salt) {
                    data = reinterpret_cast<
                            LinearProbingHashTable<uint64_t,
                            uint64_t>::EntryData *>(entry.GetPointer());
                    if (data && ht.key_equal(data->key, key)) {
                        entry = ht.entries[slot]; break; }
                }
            } else {
                data = reinterpret_cast<LinearProbingHashTable<uint64_t,
                 uint64_t>::EntryData *>(entry.GetPointer());
                if (data && ht.key_equal(data->key, key)) {
                    entry = ht.entries[slot]; break; }
            }
        ...
        LinearProbingHashTable<uint64_t, uint64_t
            >::EntryData *current = data;
        while (current) {
...
\begin{lstlisting}[language=C++, frame=single, caption={B+ Tree lookup coroutine}, label={lst:bptree_lookup_coro}, numbers=left]
...
Task lookup(const BPTree<Key_t, Value_t> &t, const auto &lookups,
    ...
    for (; idx < lookups.size(); idx += numCoroutines) {
        ...
        while (!node->isLeaf()) {
            ...
            PREFETCH(std::move(dynamic_cast<
                    PtrNode<Key_t, Value_t> *>(node)->children[i]));
            co_await std::suspend_always{};
            node = dynamic_cast<PtrNode<
                Key_t, Value_t> *>(node)->children[i];
        }
        ...
        for (size_t i = 0; i < leafNode->nr_keys; i++) {
            if (leafNode->keys[i] == key) {
                results.push_back(std::make_tuple(std::move(key),
                                  std::move(leafNode->values[i])));
...
and a base looup implementation without coroutines and without prefetching for comparison with the reference.
The primary metric used for evaluations is time taken for lookups, inluding min, max and average time of the three repetitions.
Only for scenarios where the number of threads is greater than one, only one repetition is executed.
But not only time, also additional metrics such as cycles, L1-dcache-loads, L1-dcache-load-misses, dTLB-loads, dTLB-load-misses, page-faults,
cache-misses, gigahertz, L1-data-miss-ratio, cache-hit-ratio, cache-miss-ratio and dTLB-miss-ratio are recorded.
...
For all chaining hashtables constructed the load factor is 100\% to reflect a bad hastable scenario with many collisions.

For the B+ tree, each key-value pair is of size 16 bytes (key and value each 8 bytes).
The B+ tree is configured with a default node size of 128 entries per node.
However, the balance of the B+ tree is arbitrary, as the entries are randomly inserted without any rebalancing mechanism.

For the linear probing hashtable, each entry is of size 16 bytes (key and value each 8 bytes).
The load factor is thresholded at 75\%, meaning that when the hashtable reaches 75\% capacity, it resizes by doubling its size.
...
This includes following four machines, all based on the x86-64 architecture:
\begin{itemize}
    \item \textbf{Skylake}: Intel(R) Core(TM) i7-6700 CPU @ 3.40GHz: 4 Cores, 8 Threads, 32GB main memory, L1d cache: 32KiB (per Core), L2 cache: 256KiB (per Core), L3 cache: 8MiB (shared), NUMA Nodes 1
    \item \textbf{Skylake\_X}: Intel(R) Core(TM) i9-9900X CPU @ 3.50GHz, 10 Cores, 20 Threads, 64GB main memory, L1d cache: 32KiB (per Core) , L2 cache: 1MiB (per Core), L3 cache: 19.3MiB (shared), NUMA Nodes 1
    \item \textbf{AMD}: Ryzen Threadripper 1950X : 16 Cores, 64GB main memory, L1d cache: 32KiB (per Core), L2 cache: 512KiB (per Core), L3 cache: 32MiB (shared), NUMA Nodes 2
    \item \textbf{Xeon (Sapphire Rapids)}: Intel(R) Xeon(R) Gold 6430, 32 Cores, 64 Threads, 256GB main memory, L1d cache: 48KiB (per Core), L2 cache: 2MiB (per Core), L3 cache: 60MiB (shared), NUMA Nodes 1
\end{itemize}
...
Additionally to benchmarking on different machines, multiple compilers are used to evaluate the performance of coroutine implementations across compilers.
...
the benchmarks are executed with different numbers of threads, ranging from the primarily evaluated single-threaded execution to multiple threads, e.g. 2, 4, 8, 16 threads.
...
Additionally, perf tooling and pmu-tools/toplev is used to pointwise analyze performance bottlenecks.
